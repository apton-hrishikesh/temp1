# String 1
# Note: Go through PEP8 documentation and follow the coding standard(Important)

# Basic string exercises
# Fill in the code for the functions below. main() is already set up to call the functions with a few different inputs, printing 'OK' when each function is correct. The starter code for each function includes a 'return' which is just a placeholder for your code. It's ok if you do not complete all the functions, and there are some additional functions to try in string2.py.
...
#However, if the count is 10 or more, then use the word 'many' instead of the actual count. So donuts(5) returns 'Number of donuts: 5' and donuts(23) 
#returns 'Number of donuts: many'
...
# However, if the string length is less than 2, return the empty string instead.
...
# e.g. 'babble' yields 'ba**le' .Assume that the string is length 1 or more.
# Hint: s.replace(stra, strb) returns a version of string s
# where all instances of stra have been replaced by strb.

def fix_start(s):
  a = s[0] + s[1:len(s)].replace(s[0],'*')
  return a
...
def mix_up(a, b):
    out = b[0:2]+a[2:len(a)]+" "+a[0:2]+b[2:len(b)]
    return out
