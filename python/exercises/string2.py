...
# Return the resulting string. So 'This dinner is not that bad!' : This dinner is good!.
...
def not_bad(s):
  ...
  if(x==-1 or y==2):
    ...
    a = s.replace(s[x:y], 'good')
    #print("not at ",x," bad at ",y)
    return a
  else:
    return s
...
# If the length is odd, we'll say that the extra char goes in the front half. e.g. 'abcde', the front half is 'abc', the back half 'de'. 
...
def front_back(a, b):
  ...
  if(len(a)%2==0):
    ...
    out+=a[0:int(len(a)/2)+1]
    a=a[int(len(a)/2)+1:int(len(a))]

  
  if(len(b)%2==0):
    out+=b[0:int(len(b)/2)]
    b=b[int(len(b)/2):int(len(b))]
  else:
    out+=b[0:int(len(b)/2)+1]
    b=b[int(len(b)/2)+1:int(len(b))]
